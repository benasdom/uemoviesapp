...
import { Movie } from '@/interfaces'; // adjust path to wherever your interfaces file is
...
const MovieCard = (dataItem: Movie) => {
  ...
  const displayTitle = dataItem.title ?? dataItem.name;
  const displayYear = (dataItem.release_date ?? dataItem.first_air_date)?.split('-')[0];

  return (
    <TouchableOpacity
      style={{ width: 125, backgroundColor: "rgba(250,250,250,.05)" }}
      className='flex items-center rounded-xl m-[3px]'
      onPress={() => router.push(`movies/${encodeURIComponent(dataItem.id)}?mediaType=${dataItem.mediaType ?? 'movie'}` as any)}
    >
      <ImageBackground
      ...
      <Text className="flex mt-[5] self-end px-1 text-secondary">{displayYear}</Text>
